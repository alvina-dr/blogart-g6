...
class THEMATIQUE
{
	...
	function get_1ThemeByLangue($numThem)
	{
		global $db;
		$query = 'SELECT * FROM THEMATIQUE TH INNER JOIN LANGUE LA ON TH.numLang = LA.numLang WHERE numThem = ?;';
		$result = $db->prepare($query);
	...
	function get_AllThemeByLangue()
	{
		global $db;
		$query = 'SELECT * FROM THEMATIQUE INNER JOIN LANGUE ON numLang.id = numLang.id;';
		$result = $db->query($query);
		$allStatuts = $result->fetchAll();
		return ($allStatuts);
	}
	...
	function create($numThem, $libThem, $numLang)
	{
		...
		try {
			...
			$result->bindParam(':numThem', $numAngl);
			$result->bindParam(':libThem', $libAngl);
